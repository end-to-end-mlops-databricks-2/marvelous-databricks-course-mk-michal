...
from hotel_cancellation.model_registering import get_registered_model_metric, get_latest_run_metric, register_model
...
if latest_run_metric > registered_model_metric:
    print(f"New model is better than the registered model. Registering the new model.")
    register_model(latest_run)
else:
    print(f"New model is not better than the registered model. New model is not registered")
