    ...
    categorical_transformer = Pipeline(
        steps=[
            ...
            (
                "onehot",
                OneHotEncoder(
                    drop=None, sparse_output=False, handle_unknown="ignore", feature_name_combiner=combine_feature_names
                ),
            ),
        ]
    )
...
class HotelBookingPreprocessor:
    ...
    def save(self, transformed_data: pd.DataFrame, spark, dbutils):
        ...
        (spark_df.write.format("delta").mode("overwrite").saveAsTable(Config.OUTPUT_PROCESSED_DATA_PATH))
        ...
        config_data = {"columns_config": Config.COLUMNS_CONFIG, "feature_names": self.feature_names, "run_id": run_id}
